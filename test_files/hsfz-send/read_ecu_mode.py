...
def main():
    ...
    uds_response = sendhsfz(args.ip_addr, args.diag_addr, [0x22, 0x80, 0x02])

    if len(uds_response) != 6 or uds_response[:3] != [0x62, 0x80, 0x02]:
        print("Invalid UDS response {}".format(
            " ".join("%02X" % x for x in uds_response)), file=sys.stderr)
        sys.exit(-1)

    ecu_mode = uds_response[3] * 65536 + \
        uds_response[4] * 256 + uds_response[5]

    if ecu_mode == 0:
        print("ECU mode: Plant")
    elif ecu_mode == 1:
        print("ECU mode: Engineering")
    elif ecu_mode == 2:
        print("ECU mode: Field")
    else:
        print("ECU mode: Unknown (0x{:06X})".format(ecu_mode))
