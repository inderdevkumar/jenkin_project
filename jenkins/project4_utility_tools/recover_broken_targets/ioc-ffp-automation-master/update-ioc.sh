...
echo "Copying files..."
scp -o StrictHostKeyChecking=no -i $ssh_signature_file $tar root@$target_ip:/tmp
scp -o StrictHostKeyChecking=no -i $ssh_signature_file $sig root@$target_ip:/tmp

ssh -o StrictHostKeyChecking=no -i $ssh_signature_file root@$target_ip "systemctl stop nodestatemanager"

echo "Currently installed IOC version:"
ssh -o StrictHostKeyChecking=no -i $ssh_signature_file root@$target_ip "ffptool /usr/lib/libffp_ioc.so.0 sw-version 0" | grep "SW version"

echo "Updating IOC..."
ssh -o StrictHostKeyChecking=no -i $ssh_signature_file -t root@$target_ip "ffptool /usr/lib/libffp_ioc.so.0 flash /tmp/$(basename ${tar}) /tmp/$(basename ${sig})"

# Restart Unit
ssh -o StrictHostKeyChecking=no -i $ssh_signature_file root@$target_ip "systemctl start nodestatemanager"
while [[ $(ssh -o StrictHostKeyChecking=no -i $ssh_signature_file root@$target_ip systemctl is-active --quiet nodestatemanager) -ne 0 ]]; do
	sleep 1
done

echo "Rebooting unit..."
ssh -o StrictHostKeyChecking=no -i $ssh_signature_file root@$target_ip "nsm_control --r 7 > /dev/null 2>&1"
